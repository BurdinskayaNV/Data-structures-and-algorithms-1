...
/// функция количество несъеденных неотравленных кусков в центре
static int Search_pie1(int** a, size_t n, size_t m)
{
    int count = 0;
	for (size_t i = 1; i < n; i++) // строк N
	{
...
//Число нулевых элементов в крайнем справа столбце
static int Far_right_pie(int** a, size_t n, size_t m) {
    int count = 0;
    // Проходим по крайнему правому столбцу матрицы
    for (int i = 1; i <= n; i++) {
        // Если элемент не равен 1, увеличиваем счетчик
        if (a[i][m] == 0) { count++; }
    }
    return count;
}

//Число нулевых элементов в крайнем нижней строке
static int Bottom_row_pie(int** a, size_t n, size_t m) {
    int count = 0;
...
static int Random(size_t n, int b)
{
	return 1 * (n - b) * rand() / RAND_MAX + b; // [b,n]
}
...
void Move_Computer(int** a, size_t n, size_t m, int& x, int& y)
{
	...
	int v1 = Search_pie1(a, n, m);    // проверяем матрицу a[n-1][m-1] на 0 -ые куски
	int v2 = Far_right_pie(a, n, m);  // проверяем правый столбец a[i][m] на 0 -ые куски
	int v3 = Bottom_row_pie(a, n, m); // проверяем нижнюю строку a[n][i] на 0 -ые куски

	if ((v1 != 0) && (a[x][y] != 2))    //****** для матрицы a[n-1][m-1] на 0 -ые куски
	{ 
	    do
	   {
	      x = Random(n, x);
	      y = Random(m, y);
	      bool v = Search_pie(a, n, m); // проверяем количество несъеденых кусков
	      if ((v == true) && (a[x][y] == 2)) 
		  {
			  Move_Computer(a, n, m, k, l);
		  }
	   } while (a[x][y] == 1); // пока в центре не будет = 1
	}
	
	if ((v1 == 0) && (v2 >= 2) && (v3 >= 2)) // в центре пусто,края съедобные
	{ 
	    x = n - 2; y = m;  // правый столбец a[i][m]
	}
	if ((v1 == 0) && ((v2 == 0) || (v2 == 1)) && (v3 >= 2)) // в центре пусто, нижняя строка съедобная и справа 2 нижних
	{ 
	    x = n; y = m - 2;  // нижнюю строку a[n][i]
	}
	if ((v1 == 0) && (v2 >= 2) && ((v3 == 0) || (v3 == 1))) // в центре пусто, правая крайняя съедобная и нижняя строка 2 нижних
	{ 
	    x = n; y = m - 2; // нижнюю строку a[n][i]
	}
	if ((v1 == 0) && (v2 == 1) && (v3 == 1))  // в центре пусто, правая крайняя съедобная 2 снизу и нижняя строка 2 нижних
	{ 
	    x = n; y = m - 1; // нижнюю строку a[n][i]
	}
	if ((v1 == 0) && (v2 == 0) && (v3 == 1))  // в центре пусто, съедобная 2 снизу в нижней строке
	{ 
	    x = n; y = m - 1; // нижнюю строку a[n][i]
	}
	if ((v1 == 0) && (v2 == 1) && (v3 == 0))  // в центре пусто, съедобная 2 снизу правая крайняя
	{ 
	    x = n - 1; y = m;  // правый столбец a[i][m]
	}
	if ((v1 == 0) && (v2 == 0) && (v3 == 0)) // только отравленный
	{
	    x = n; y = m;  // отравленный кусок
	}
}
