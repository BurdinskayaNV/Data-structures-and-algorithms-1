...
/// если index будет найден, то вернется его индекс,
//  если нет, то вернется число -1, 
size_t Search_predicate(TArray* arr, size_t n, function<bool(TArray&)> f) {
	for (size_t i = 0; i < n; i++)
		if (f(arr[i])) return i;
	return -1;
}

//---тест работы функции Search_predicate
void Test_Search_predicate() {
	int* a = new int[5] { 0, 1, 2, 3, 4};
	size_t m = Search_predicate<int>(a, 5, [](int& x) { return !(x % 2 == 0); }); //ищет первое нечётное число
	assert(m == 1);
	delete[] a;

	double* a1 = new double[5] { -1.534, -2.54, -564534.867, 8665.0, 5.123};
	m = Search_predicate<double>(a1, 5, [](double& x) { return (x > 0); }); //ищет первое неотрицательное число
	assert(m == 3);
	delete[] a1;

	int* a2 = new int[8] {8, 7, 6, 5, 4, 3, 2, 1};
	m = Search_predicate<int>(a2, 8, [](int& x) {return (x % 4 == 0); }); //ищет число кратное 4
	assert(m == 0);
	delete[] a2;
	cout << "--- Test_Search_predicate() выполнен успешно --------" << endl;
}
